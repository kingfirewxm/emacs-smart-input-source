...
(ert-deftest sis-test-authoritative-cursor-mode-does-not-enable-refresh ()
  (sis-test-with-backend 'sis
    (cl-letf (((symbol-function 'terminal-list) (lambda () nil)))
      (unwind-protect
          (progn
...
(ert-deftest sis-test-terminal-cursor-uses-theme-reset-for-english ()
  (let ((sis--current 'english)
        (sis-other-cursor-color "pink")
        (sent nil))
    (cl-letf (((symbol-function 'terminal-live-p) (lambda (_) t))
              ((symbol-function 'send-string-to-terminal)
               (lambda (sequence &optional terminal)
                 (push (list sequence terminal) sent))))
      (sis--update-terminal-cursor-color 'tty)
      (setq sis--current 'other)
      (sis--update-terminal-cursor-color 'tty)
      (should
       (equal (nreverse sent)
              (list (list sis--terminal-cursor-reset 'tty)
                    (list "\e]12;pink\a" 'tty)))))))

(ert-deftest sis-test-terminal-cursor-lifecycle-is-symmetric ()
  (let ((sis--current 'other)
        (sis-other-cursor-color "pink")
        (sis-global-cursor-color-mode t)
        (reset-strings '("existing"))
        (sent nil))
    (cl-letf (((symbol-function 'terminal-live-p) (lambda (_) t))
              ((symbol-function 'send-string-to-terminal)
               (lambda (sequence &optional _terminal)
                 (push sequence sent)))
              ((symbol-function 'terminal-parameter)
               (lambda (_terminal parameter)
                 (when (eq parameter 'tty-mode-reset-strings)
                   reset-strings)))
              ((symbol-function 'set-terminal-parameter)
               (lambda (_terminal parameter value)
                 (when (eq parameter 'tty-mode-reset-strings)
                   (setq reset-strings value)))))
      (sis--setup-terminal-cursor 'tty)
      (sis--setup-terminal-cursor 'tty)
      (should (= (cl-count sis--terminal-cursor-reset reset-strings
                           :test #'equal)
                 1))
      (should (equal (car sent) "\e]12;pink\a"))
      (sis--teardown-terminal-cursor 'tty)
      (should (equal (car sent) sis--terminal-cursor-reset))
      (should-not (member sis--terminal-cursor-reset reset-strings)))))

(ert-deftest sis-test-terminal-cursor-resume-respects-mode ()
  (let ((sis--current 'english)
        (sis-global-cursor-color-mode nil)
        (sent nil))
    (cl-letf (((symbol-function 'terminal-live-p) (lambda (_) t))
              ((symbol-function 'send-string-to-terminal)
               (lambda (sequence &optional _terminal)
                 (push sequence sent))))
      (sis--resume-terminal-cursor 'tty)
      (should-not sent)
      (setq sis-global-cursor-color-mode t)
      (sis--resume-terminal-cursor 'tty)
      (should (equal sent (list sis--terminal-cursor-reset))))))
