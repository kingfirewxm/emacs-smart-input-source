...
(use-package sis
  ...
  ;; 可选：如果 Emacs 中的所有输入法切换都经过 SIS，可让 SIS 成为状态权威。
  ;; 自动后端读取会被禁用，`sis-get' 仍可用于显式诊断。
  ;; (setq sis-input-source-state-authority 'sis)

  ;; 可选：操作系统没有按应用保存输入法状态时，让 SIS 处理失焦与恢复。
...
| ~sis-input-source-state-authority~ | 被跟踪状态的权威来源            | ~'external~                          |
|----------------------------+----------------------------------------+--------------------------------------|
...
| ~sis-reapply-current-input-source~ | 不读取后端，重新应用已跟踪状态 |
|--------------------------+--------------------------------|
...
| ~sis-respect-application-focus~                 | 处理应用焦点切换时的输入源                | ~nil~                  |
