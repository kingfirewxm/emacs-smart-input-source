...
(use-package sis
  ...
  ;; Optional: use this when every input source change in Emacs goes through
  ;; SIS. Automatic backend reads are disabled, while `sis-get' remains an
  ;; explicit diagnostic command.
  ;; (setq sis-input-source-state-authority 'sis)

  ;; Optional: let SIS restore state across application focus changes when the
  ;; operating system does not already preserve per-application state.
  ;; (setq sis-respect-application-focus t)
...
| ~sis-input-source-state-authority~ | Authority for the tracked state          | ~'external~                          |
|----------------------------+----------------------------------------------------+--------------------------------------|
...
| ~sis-reapply-current-input-source~ | Reapply tracked state without reading the backend |
|--------------------------+------------------------------------------------------------|
...
| ~sis-respect-application-focus~                 | Handle input source across application focus           | ~nil~                  |
